...
if [ ! -f "$HISTORY_DIR/historical-data.json" ]; then
  ...
  # Calculate next run date (tomorrow at the same time)
  NEXT_RUN_DATE=$(date -d "tomorrow" +"%Y-%m-%d")
  CURRENT_YEAR=$(date +"%Y")

  # Use template instead of inline HTML
  TEMPLATE_DIR="$(dirname "$0")/../templates"
  cat "$TEMPLATE_DIR/empty-trends.template.html" | \
    sed -e "s|\${NEXT_RUN_DATE}|$NEXT_RUN_DATE|g" \
        -e "s|\${CURRENT_YEAR}|$CURRENT_YEAR|g" \
    > "$OUTPUT_DIR/trends.html"

  echo "Created empty trends dashboard."
  exit 0
fi
